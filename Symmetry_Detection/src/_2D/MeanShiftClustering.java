...
public class MeanShiftClustering implements Clustering {
	...
	public Cluster<Point_2> clusters=new Cluster<Point_2>();
	...
	RangeSearch<Point_2> Rs;  // data structure for nearest neighbor search
	...
	void initMSC (PointCloud n, PointCloud s, double cr, double ar, double ir, double mr){
		...
		sqInflRad = ir*ir;
		sqMergeRad = mr*mr;
		Rs = new SlowRangeSearch<Point_2>(N);
	}
	 ...
	 */
	public PointCloud detectCluster (Point_2 seed, int clusterIndex) {
		//COMPLETE
		LinkedList<Point_2> currentSeeds = new LinkedList<Point_2>(); //La liste des xi successifs
	 ...
	 */
	public int mergeCluster (PointCloud cluster, Point_2[] clusterCenters) {
		//COMPLETE
		int i=0;
	 ...
	 * 
	 * @return Point_2[] an array of points, of size n .First i elements are cluster centers (non null points), remaining n-i elements must be null
	 */
	public Point_2[] computeClusters () {
		
		//On crée le tableau clusterCenters
		int nbPoints = PointCloud.size(N);
		Point_2[] clusterCenters = new Point_2[nbPoints];
		...
		//Nombre de clusters
		int nbClusters = 0;
		//Tant qu'il existe des graines non classifies, on choisit la 1ère
		while(seeds!=null){
			if(this.clusters.getClusterIndex(seeds.p) == -1){
				PointCloud l = detectCluster(seeds.p, nbClusters);
				//Si on ne peut pas fusionner ce cluster avec un cluster déjà existant
